          ...
          <div className="grid gap-5 lg:grid-cols-[1.12fr_0.88fr] lg:items-start lg:gap-8">
            <div className="grid gap-4 lg:gap-5">
              <div className="grid gap-3 md:grid-cols-[5.5rem_1fr] lg:gap-4">
                ...
                <div data-product-detail-image className="relative min-h-[20rem] overflow-hidden rounded-2xl border-0 bg-transparent shadow-none md:min-h-[24rem] lg:min-h-[28rem] lg:rounded-[2rem] lg:border lg:border-[#E5D8C7] lg:bg-[#FFF9F1] lg:shadow-[0_24px_70px_rgba(84,61,35,0.12)]">
            ...
            <div className="rounded-none border-0 bg-transparent p-0 shadow-none md:p-0 lg:sticky lg:top-28 lg:rounded-2xl lg:border lg:border-[#E5D8C7] lg:bg-[#FFF9F1] lg:p-6 lg:shadow-[0_18px_60px_rgba(84,61,35,0.08)]">
              ...
              <div className="mt-5 grid gap-2 pb-16 text-sm lg:mt-6 lg:gap-3 lg:pb-0">
                <InfoLine icon={<Truck className="mt-0.5 h-4 w-4 shrink-0 text-[#B68A45]" />} text="Enjoy free delivery and free returns on selected orders." />
                <InfoLine icon={<MapPin className="mt-0.5 h-4 w-4 shrink-0 text-[#B68A45]" />} text="Installation support available for eligible purifier models." />
                <InfoLine icon={<Heart className="mt-0.5 h-4 w-4 shrink-0 text-[#B68A45]" />} text="Genuine Priya's Aqua Fresh products and spare parts." />
              </div>
            </div>
          </div>
            ...
            <section data-related-products-section className="mt-12 border-t border-[#E5D8C7] pt-8 lg:mt-16 lg:pt-10">
              <div className="mb-5 flex flex-col justify-between gap-3 md:mb-7 md:flex-row md:items-end">
                <div>
                  ...
                  <h2 className="mt-2 font-serif text-2xl font-semibold tracking-tight text-[#1D2D2E] md:text-5xl">You May Also Like This Product</h2>
                  <p className="mt-2 max-w-2xl text-sm font-semibold leading-6 text-[#5A6362]">Similar Priya's Aqua Fresh products selected from the catalog.</p>
                </div>
                <Link href="/products" className="inline-flex w-fit rounded-full border border-[#C59A55] bg-[#FFF9F1] px-5 py-2.5 text-sm font-black text-[#9B7137] transition hover:bg-[#F5E9D8]">View All Products</Link>
              </div>
          ...
          {details.map((item, index) =>
            item.kind === "point" ? (
              <div key={`${item.text}-${index}`} className="flex gap-3 text-sm font-semibold leading-7 text-[#5A6362]">
                ...
                <p>{item.label ? <span className="font-black text-[#1D2D2E]">{item.label}: </span> : null}{item.text}</p>
              </div>
            ) : (
              <p key={`${item.text}-${index}`} className="text-sm font-semibold leading-7 text-[#5A6362]">{item.text}</p>
            ),
          )}
        </div>
      ) : null}
