              ...
              >
                ...
                <div className="relative flex h-full items-center gap-5">
                  <div className="relative h-32 w-36 shrink-0">
                    ...
                    <Image src={category.image} alt={category.name} fill sizes="144px" className="object-contain drop-shadow-[0_14px_22px_rgba(84,61,35,0.22)] transition duration-500 group-hover:-translate-y-1 group-hover:scale-105" unoptimized />
                  </div>

                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-black uppercase tracking-[0.2em] text-[#B68A45]">Explore Range</p>
                    <h2 className="mt-2 text-xl font-black leading-tight text-[#1D2D2E] transition group-hover:text-[#0A3A38] md:text-2xl">{category.name}</h2>
                    <p className="mt-3 text-sm font-semibold text-[#5A6362]">{category.productCount > 0 ? `${category.productCount} products available` : "Products coming soon"}</p>
