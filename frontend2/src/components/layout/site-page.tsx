...
export function SitePage({ eyebrow, title, description, compactHero = false, children }: SitePageProps) {
  return (
    <>
      <Header />
      ...
      <section className={`relative overflow-hidden px-5 md:px-8 ${compactHero ? "pb-5 pt-7 md:pb-6 md:pt-8" : "pb-8 pt-12 md:pt-14"}`}>
        ...
        <div className="mx-auto max-w-7xl">
          ...
          {description ? <p className={`relative max-w-3xl font-semibold text-[#5A6362] ${compactHero ? "mt-3 text-sm leading-6" : "mt-5 text-lg leading-8"}`}>{description}</p> : null}
        </div>
      </section>
      {children}
      <Footer />
