export function SkeletonBlock({ className = "" }: { className?: string }) {
  return <div className={`animate-pulse rounded-lg bg-[#E8DCCB] ${className}`} />;
}

function ProductCardSkeleton() {
  return (
    <article className="overflow-hidden rounded-[0.9rem] border border-[#E8DCCB] bg-[#FFFBF6] text-center shadow-[0_8px_24px_rgba(70,50,25,0.07)]">
    ...
    <section className="px-5 pb-20 md:px-8">
      <div className="mx-auto grid max-w-7xl items-start gap-8 lg:grid-cols-[18rem_1fr]">
        ...
        <div className="min-w-0">
          <div className="fixed left-0 top-1/2 z-40 inline-flex -translate-y-1/2 items-center gap-2 rounded-r-full border border-l-0 border-[#D9C5AB] bg-[#0A3A38] px-3.5 py-2.5 shadow-[0_10px_24px_rgba(10,36,38,0.18)] lg:hidden">
        ...
        <div className="grid gap-5 lg:grid-cols-[1.12fr_0.88fr] lg:items-start lg:gap-8">
          <div className="grid gap-4 lg:gap-5">
            <div className="grid gap-3 md:grid-cols-[5.5rem_1fr] lg:gap-4">
              ...
              <div className="relative min-h-[20rem] overflow-hidden rounded-2xl border-0 bg-transparent shadow-none md:min-h-[24rem] lg:min-h-[28rem] lg:rounded-[2rem] lg:border lg:border-[#E5D8C7] lg:bg-[#FFF9F1] lg:shadow-[0_24px_70px_rgba(84,61,35,0.12)]">
                <span className="absolute inset-x-16 bottom-10 h-16 rounded-full bg-[#0A3A38]/10 blur-2xl" />
...
export function CategoryGridSkeleton({ count = 6 }: { count?: number }) {
  return (
    <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="min-h-[180px] rounded-[1.35rem] border border-[#E5D8C7] bg-[#FFF9F1] p-5 shadow-[0_14px_42px_rgba(84,61,35,0.08)]">
...
export function OrderListSkeleton({ count = 3 }: { count?: number }) {
  return (
    <div className="grid gap-4">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="grid grid-cols-[4.75rem_1fr] gap-3 rounded-2xl border border-[#E8DCCB] bg-[#FFF9F1] p-3 shadow-[0_10px_30px_rgba(84,61,35,0.06)] sm:grid-cols-[96px_1fr_auto] sm:gap-4 sm:p-4">
