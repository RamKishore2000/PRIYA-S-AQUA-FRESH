...
export function BuyingBenefits() {
  return (
    <section data-home-reveal className="relative overflow-hidden bg-[#FFFFFF] px-5 py-12 md:px-8 md:py-16">
      <div className="mx-auto max-w-7xl">
        <div data-reveal-item className="grid gap-8 rounded-[1.1rem] border border-[#D8EAF8] bg-[#FFFFFF] p-6 shadow-[0_8px_24px_rgba(0,87,200,0.07)] lg:grid-cols-[0.78fr_1.22fr] lg:items-start">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.24em] text-[#0057C8]">Why Choose Priya&apos;s Aqua Fresh</p>
          ...
          <div className="grid gap-x-10 gap-y-8 sm:grid-cols-2">
            {benefits.map((benefit, index) => (
              <div key={benefit.title} data-reveal-item className="rounded-xl border border-[#D8EAF8] bg-[#F8EFE5] p-5">
                <span className="grid h-10 w-10 place-items-center rounded-lg bg-[#FFFFFF] text-sm font-black text-[#2B6260]">{String(index + 1).padStart(2, "0")}</span>
