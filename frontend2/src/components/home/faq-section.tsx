...
export function FaqSection() {
  ...
  return (
    <section data-home-reveal className="bg-[#F3FAFF] px-5 py-16 md:px-8 md:py-20">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.78fr_1.22fr]">
        <div data-reveal-item>
          <p className="text-xs font-black uppercase tracking-[0.24em] text-[#0057C8]">FAQ</p>
          <h2 className="mt-3 font-serif text-4xl font-semibold text-[#102033] md:text-5xl">Frequently Asked Questions</h2>
          <p className="mt-5 text-lg leading-8 text-[#40576C]">Clear answers for product selection, service and support.</p>
        </div>
            ...
            <div key={faq.question} data-reveal-item className="overflow-hidden rounded-[0.85rem] border border-[#E6DACB] bg-[#FFFFFF] shadow-[0_8px_24px_rgba(0,87,200,0.06)]">
              ...
              <div className={`grid transition-all duration-500 ${open === index ? "grid-rows-[1fr]" : "grid-rows-[0fr]"}`}>
                <div className="overflow-hidden">
                  <p className="px-5 pb-5 leading-7 text-[#40576C]">{faq.answer}</p>
                </div>
              </div>
            </div>
          ))}
