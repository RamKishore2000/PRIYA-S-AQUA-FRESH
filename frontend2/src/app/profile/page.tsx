    ...
    <SitePage eyebrow="Account" title="My Profile" description="Manage your account details and view your order history.">
      <section className="px-4 pb-28 md:px-8">
        <div className="mx-auto grid max-w-7xl gap-4 lg:grid-cols-[340px_1fr] lg:gap-6">
          <aside className="h-max rounded-2xl border border-[#E8DCCB] bg-[#FFF9F1] p-4 shadow-[0_10px_30px_rgba(84,61,35,0.06)] lg:p-5">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate font-black text-[#1D2D2E]">{user.mobile || "Customer"}</p>
                <p className="truncate text-sm font-semibold text-[#5A6362]">{user.role}</p>
              </div>
            </div>
            <div className="mt-5 grid grid-cols-3 gap-2 text-xs sm:text-sm lg:grid-cols-1">
              <Link href="/profile" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl bg-[#F5E9D8] px-2 py-2 text-center font-black text-[#0A3A38] lg:justify-start lg:gap-2 lg:px-3"><Home className="h-4 w-4" /> Profile</Link>
              <Link href="/profile/orders" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl px-2 py-2 text-center font-black text-[#5A6362] hover:bg-[#F5E9D8] hover:text-[#0A3A38] lg:justify-start lg:gap-2 lg:px-3"><Package className="h-4 w-4" /> Order History</Link>
              <Link href="/wishlist" className="flex min-h-11 items-center justify-center gap-1.5 rounded-xl px-2 py-2 text-center font-black text-[#5A6362] hover:bg-[#F5E9D8] hover:text-[#0A3A38] lg:justify-start lg:gap-2 lg:px-3"><Heart className="h-4 w-4" /> Wishlist</Link>
            </div>
          </aside>

          <div className="space-y-6">
            <section className="rounded-2xl border border-[#E8DCCB] bg-[#FFF9F1] p-4 shadow-[0_10px_30px_rgba(84,61,35,0.06)] lg:p-6">
            ...
            <section className="rounded-2xl border border-[#E8DCCB] bg-[#FFF9F1] p-4 shadow-[0_10px_30px_rgba(84,61,35,0.06)] lg:p-6">
              <h2 className="text-xl font-black text-[#1D2D2E]">Important Links</h2>
...
function Info({ label, value }: { label: string; value?: string }) {
  return (
    <div className="rounded-xl border border-[#E5D8C7] bg-white p-3 md:p-4">
      <p className="text-xs font-black uppercase tracking-[0.16em] text-[#B68A45]">{label}</p>
      <p className="mt-1 break-words font-black text-[#1D2D2E]">{value || "-"}</p>
    </div>
  );
}
...
function LegalLink({ href, label, icon }: { href: string; label: string; icon: ReactNode }) {
  return (
    <Link href={href} className="flex min-h-12 items-center gap-3 rounded-xl border border-[#E5D8C7] bg-white px-4 py-3 text-sm font-black text-[#1D2D2E] transition hover:border-[#B68A45] hover:text-[#0A3A38]">
      <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[#F5E9D8] text-[#0A3A38]">{icon}</span>
      {label}
    </Link>
  );
}
