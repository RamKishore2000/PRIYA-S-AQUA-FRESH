  ...
  for (const row of rows) {
    if (!products.has(row.id)) {
      products.set(row.id, {
        ...
        sortOrder: Number(row.product_sort_order ?? 999),
        status: row.status,
        category: {
          id: row.category_id,
...
const productSelect = `
  SELECT p.id, p.category_id, p.subcategory_id, p.name, p.slug, p.sku, p.description, p.rating, p.review_count, p.sort_order AS product_sort_order, p.status, p.created_at, p.updated_at,
...
async function findAll({ includeInactive = false, categorySlug, subcategorySlug, searchTerm } = {}) {
  const conditions = [];
  const params = [];
  ...
  if (!includeInactive) {
    ...
    conditions.push("(sc.id IS NULL OR sc.status = 'ACTIVE')");
  }
  if (categorySlug) {
  ...
  if (searchTerm) {
    conditions.push("(p.name LIKE ? OR p.sku LIKE ? OR p.description LIKE ? OR c.name LIKE ? OR sc.name LIKE ?)");
  ...
  const [rows] = await pool.execute(
    `${productSelect}
     ${where}
     ORDER BY p.sort_order ASC, p.created_at DESC, pi.sort_order ASC, pi.id ASC`,
    params,
  );
...
async function createProduct(payload) {
  ...
  try {
    ...
    const [productResult] = await connection.execute(
      `INSERT INTO products (category_id, subcategory_id, name, slug, sku, description, rating, review_count, sort_order, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [payload.categoryId, payload.subcategoryId || null, payload.name, payload.slug, payload.sku, payload.description, payload.rating, payload.reviewCount, payload.sortOrder, payload.status],
    );
...
async function updateProduct(id, payload) {
  ...
  try {
    ...
    await connection.execute(
      `UPDATE products
       SET category_id = ?, subcategory_id = ?, name = ?, slug = ?, sku = ?, description = ?, rating = ?, review_count = ?, sort_order = ?, status = ?
       WHERE id = ?`,
      [payload.categoryId, payload.subcategoryId || null, payload.name, payload.slug, payload.sku, payload.description, payload.rating, payload.reviewCount, payload.sortOrder, payload.status, id],
    );
