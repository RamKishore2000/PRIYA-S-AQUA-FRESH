        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
              ...
              {order.products.map((product) => (
                <div key={product.id} className="grid gap-4 p-5 sm:grid-cols-[80px_1fr_auto]">
                  <div className="relative h-20 w-20 overflow-hidden rounded-md border border-slate-200 bg-slate-50">
                    <Image src={product.imageUrl || "/file.svg"} alt={product.productName} fill className="object-contain p-1.5" unoptimized />
                  </div>
                  <div>
                    <p className="font-bold text-slate-950">{product.productName}</p>
                    <p className="mt-1 text-sm text-slate-500">SKU: {product.productSku}</p>
                    <p className="mt-2 text-sm font-semibold text-slate-700">Qty: {product.quantity}</p>
                  </div>
                  <div className="font-bold text-slate-950 sm:text-right">
                    <p>{formatCurrency(product.lineTotal)}</p>
                    <p className="mt-1 text-xs text-slate-500">{formatCurrency(product.unitPrice)} each</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <aside className="space-y-5">
            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <button type="button" onClick={saveStatus} disabled={saving} className="mt-3 h-10 w-full rounded-md bg-teal-600 px-4 text-sm font-bold text-white transition hover:bg-teal-700 disabled:opacity-60">
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <Info label="Mobile" value={order.buyerMobile} />
              <Info label="Email" value={order.buyerEmail || "-"} />
              <Info label="Role" value={order.role} />
            </section>
            ...
            <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <div className="mt-4 space-y-3 text-sm">
                ...
                <div className="border-t border-slate-200 pt-3">
                  <Row label="Total" value={formatCurrency(order.amount)} strong />
                </div>
              </div>
            </section>
          </aside>
